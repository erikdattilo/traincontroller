...
// 	wxString    notice;
// 
// 	notice.Printf(wxT("%s - %snn"), program_name.c_str(), __DATE__);	// L("Traindir 3.0nn");
// 	notice += L("Created by");
// 	notice += wxT(" Giampiero Caprinonn");
// 	notice += L("This is free software, released under thenGNU General Public License Version 2.nThe author declines any responsibility for any damagenthat might occur from the use of this program.");
// 	notice += wxT("    nn");
// 	notice += L("This is a game, and is not intended tonbe used to actually control train traffic.");
// 
// 	wxMessageBox(notice);
// //	ShowWelcomePage();
// }
...
// 	fileMenu->Append(wxID_OPEN, L("&Open...tCtrl-O"), L("Open a simulation file."));
...
// 	fileMenu->AppendSeparator();
// 	fileMenu->Append(wxID_EXIT, L("E&xittAlt-F4"), L("Quit this program."));
// 
// 	wxMenu	*editMenu = new wxMenu;
// 	editMenu->Append(MENU_EDIT, L("&EdittCtrl-E"), L("Enter/exit layout editor."));
...
// 	runMenu->Append(MENU_START, L("&StarttCtrl-S"), L("Start/stop the simulation."));
// 	runMenu->Append(MENU_GRAPH, L("&GraphtCtrl-G"), L("Show the timetable graph."));
// 	runMenu->Append(MENU_LATEGRAPH, L("&Late GraphtCtrl-L"), L("Show accumulation of late minutes over time."));
...
// 	runMenu->Append(MENU_FAST, L("&FasttCtrl-X"), L("Speed up the simulation."));
// 	runMenu->Append(MENU_SLOW, L("&SlowtCtrl-Z"), L("Slow down the simulation."));
// 	runMenu->Append(MENU_SKIP, L("S&kip aheadtCtrl-K"), L("Skip to 3 minutes before next event."));
// 	runMenu->Append(MENU_STATION_SCHED, L("S&tation scheduletF6"), L("Show the train schedule of each station."));
// 	runMenu->Append(MENU_SETGREEN, L("Set sig. to green"), L("Set all automatic signals to green."));
// 	runMenu->Append(MENU_SELECT_ITIN, L("Select ItinerarytCtrl-I"), L("Select an itinerary by name."));
// 	runMenu->Append(MENU_PERFORMANCE, L("&Performance"), L("Show performance data."));
// 
// 	m_viewMenu = new wxMenu;
// 	m_viewMenu->Append(MENU_SHOW_LAYOUT, L("Show layouttF3"), L("Forcibly show the layout window."));
// 	m_viewMenu->Append(MENU_SHOW_SCHEDULE, L("Show scheduletF4"), L("Forcibly show the schedule window."));
// 	m_viewMenu->Append(MENU_INFO_PAGE, L("Show info pagetF5"), L("Shows the scenario info page, if available."));
...
// 	helpMenu->Append(wxID_ABOUT, L("WelcometF1"), L("Show the welcome page."));
