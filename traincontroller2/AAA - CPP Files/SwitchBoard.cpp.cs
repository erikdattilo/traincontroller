...
// 	++xMax;
// 	++yMax;
// 	str.Append(wxT("<table class="switchboard">n"));
// 	for(y = 0; y < yMax; ++y) {
// 	    str.Append(wxT("<tr>n"));
// 	    for(x = 0; x < xMax; ++x) {
// 		cell = grid[x][y];
// 		if(!cell)
// 		    str.Append(wxT("<td class="empty">&nbsp;</td>n"));
// 		else
// 		    cell->toHTML(str, urlBase);
// 	    }
// 	    str.Append(wxT("</tr>n"));
// 	}
// 	str.Append(wxT("</table>n"));
// 	return true;
// }
// 
// 
...
// 
// 	for(sb = switchBoards; sb; sb = sb->_next) {
// 	    file.Write(wxString::Format(wxT("(switchboard %s)n"), sb->_fname.c_str()));
// 	    wxFFile file;
// 	    if(!file_create(sb->_fname, wxT(".swb"), file))
// 		break;
// 	    SwitchBoardCellAspect *asp;
// 	    for(asp = sb->_aspects; asp; asp = asp->_next) {
// 		file.Write(wxString::Format(wxT("Aspect: %sn"), asp->_name));
// 		file.Write(wxString::Format(wxT("Bgcolor: %snn"), asp->_bgcolor));
// 	    }
// 	    SwitchBoardCell *cell;
//             file.Write(wxString::Format(wxT("Name: %sn"), sb->_name.c_str()));
// 	    for(cell = sb->_cells; cell; cell = cell->_next) {
// 		file.Write(wxString::Format(wxT("Cell: %d,%dn"), cell->_x, cell->_y));
// 		file.Write(wxString::Format(wxT("Itinerary: %sn"), cell->_itinerary.c_str()));
// 		file.Write(wxString::Format(wxT("Text: %snn"), cell->_text.c_str()));
// 	    }
// 	    file.Close();
// 	}
// }
