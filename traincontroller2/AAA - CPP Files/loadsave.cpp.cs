...
// #include "GTFS.h"
// 
// extern	void	*w_train_pmap_default[NTTYPES], *w_train_pmap[NTTYPES];
// extern	void	*e_train_pmap_default[NTTYPES], *e_train_pmap[NTTYPES];
// extern	void	*w_car_pmap_default[NTTYPES], *w_car_pmap[NTTYPES];
// extern	void	*e_car_pmap_default[NTTYPES], *e_car_pmap[NTTYPES];
// 
// Path	*paths;
...
// 		wxStrcat(p, wxT("n"));	/* put it back, since we removed it */
// 		tl->txt = wxStrdup(p);
// 		if(!track_info)
// 		    track_info = tl;
// 		else
...
// 		continue;
// 	    }
// 	    if(!wxStrncmp(buff, wxT("Routes:"), 7)) {
// 		for(p = buff + 7; *p == ' '; ++p);
//                 if(!*p)
//                     continue;
//                 gtfs->SetOurRoutes(p);
...
// 		continue;
// 	    }
// 	    if(!wxStrncmp(buff, wxT("Cancel:"), 7)) {
// 		for(p = buff + 7; *p == ' '; ++p);
// 		if(!*p)
// 		    continue;
...
// 		continue;
// 	    }
//             if(!wxStrncmp(p, wxT("StartDelay: "), 12)) {
//                 t->myStartDelay = wxAtoi(p + 12);
//                 continue;
//             }
//             if(!wxStrncmp(p, wxT("AccelRate: "), 11)) {
...
// 		    file.Write(wxString::Format(wxT("%sn"), t->station));
// 		else
// 		    file.Write(wxString::Format(wxT("nonamen")));
// 		break;
// 
// 	    case SWITCH:
...
// 		file.Write(wxString::Format(wxT("n")));
// 		break;
// 
// 	    case PLATFORM:
// 		file.Write(wxString::Format(wxT("3,%d,%d,%dn"), t->x, t->y, t->direction == W_E ? 0 : 1));
// 		break;
// 
// 	    case TEXT:
...
// 		    file.Write(wxString::Format(wxT("5,%d,%d,0,@%d,%d,%sn"), t->x, t->y,
//                             t->wlinkx, t->wlinky, t->station + i + 1));
//                 else
// 		    file.Write(wxString::Format(wxT("5,%d,%d,0,%sn"), t->x, t->y, t->station + i + 1));
// 		break;
// 
// 	    case ITIN:
// 		file.Write(wxString::Format(wxT("8,%d,%d,%d,%sn"), t->x, t->y, t->direction, t->station));
// 		break;
// 
// 	    case TRIGGER:
...
// 	    file.Write(wxString::Format(wxT("6,0,0,0,%sn"), tl->txt));
// 
...
// 		    file.Write(wxString::Format(wxT("(attributes %d,%dnicons:"), t->x, t->y));
// 		    for(int x = 0; ;) {
...
// 
// 		if(t->stateProgram) {
// 		    file.Write(wxString::Format(wxT("(script %d,%dn%s)n"), t->x, t->y, t->stateProgram));
// 		}
//                 if(t->power && *t->power) {
//                     file.Write(wxString::Format(wxT("(attributes %d,%dnpower:%sn)n"), t->x, t->y, t->power));
//                 }
//                 if(t->gauge) {
//                     file.Write(wxString::Format(wxT("(attributes %d,%dngauge:%sn)n"), t->x, t->y, t->gauge));
//                 }
// 
// 	    case TSIGNAL:
...
//                 }
//                 if(t->_lockedBy) {
//                     file.Write(wxString::Format(wxT("(attributes %d,%dnlocked %sn)n"), t->x, t->y, t->_lockedBy));
//                 }
//                 if(t->flags & TFLG_DONTSTOPSHUNTERS) {
//                     file.Write(wxString::Format(wxT("(attributes %d,%dndontstopshuntersn)n"), t->x, t->y));
//                 }
//                 if(t->_intermediate) {
//                     file.Write(wxString::Format(wxT("(attributes %d,%dnintermediate %dn)n"),
...
// 	    case TEXT:
// 		if(t->invisible) {
// 		    file.Write(wxString::Format(wxT("(attributes %d,%dnhiddenn)n"), t->x, t->y));
// 		}
// 		continue;
// 	    }
// 	}
...
// 	if(outFormat == OUT_TEXT) {
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Wrong destinations"),
// 		perf_tot.wrong_dest, perf_vals.wrong_dest,
// 		perf_tot.wrong_dest * perf_vals.wrong_dest);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Late trains"),
// 		perf_tot.late_trains, perf_vals.late_trains,
// 		perf_tot.late_trains * perf_vals.late_trains);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Wrong platforms"),
// 		perf_tot.wrong_platform, perf_vals.wrong_platform,
// 		perf_tot.wrong_platform * perf_vals.wrong_platform);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Commands denied"),
// 		perf_tot.denied, perf_vals.denied,
// 		perf_tot.denied * perf_vals.denied);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Trains waiting at signals"),
// 		perf_tot.waiting_train, perf_vals.waiting_train,
// 		perf_tot.waiting_train * perf_vals.waiting_train);
// 
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Thrown switches"),
// 		perf_tot.thrown_switch, perf_vals.thrown_switch,
// 		perf_tot.thrown_switch * perf_vals.thrown_switch);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Cleared signals"),
// 		perf_tot.cleared_signal, perf_vals.cleared_signal,
// 		perf_tot.cleared_signal * perf_vals.cleared_signal);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Reversed trains"),
// 		perf_tot.turned_train, perf_vals.turned_train,
// 		perf_tot.turned_train * perf_vals.turned_train);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dn"), L("Missed station stops"),
// 		perf_tot.nmissed_stops, perf_vals.nmissed_stops,
// 		perf_tot.nmissed_stops * perf_vals.nmissed_stops);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("%-40s : %5d x %3d = %-5dnn"), L("Wrong stock assignments"),
// 		perf_tot.wrong_assign, perf_vals.wrong_assign,
// 		perf_tot.wrong_assign * perf_vals.wrong_assign);
// 	    page.Add(buff);
// 	} else {
// 	    page.Add(wxT("</td><td valign=top>n"));
// 	    buff.Printf(wxT("<table><tr><td valign=top>%s</td>n"), L("Wrong destinations"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.wrong_dest, perf_vals.wrong_dest);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td>%d</td></tr>n"), perf_tot.wrong_dest * perf_vals.wrong_dest);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Late trains"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.late_trains, perf_vals.late_trains);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.late_trains * perf_vals.late_trains);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Wrong platforms"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top align=right>%d&nbsp;x</td><td valign=top align=right>%d&nbsp;=</td>"),
// 			    perf_tot.wrong_platform, perf_vals.wrong_platform);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.wrong_platform * perf_vals.wrong_platform);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Commands denied"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.denied, perf_vals.denied);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.denied * perf_vals.denied);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Trains waiting at signals"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.waiting_train, perf_vals.waiting_train);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.waiting_train * perf_vals.waiting_train);
// 	    page.Add(buff);
// 
// 	    buff.Printf(wxT("</table></td><td valign=top><table>"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Thrown switches"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.thrown_switch, perf_vals.thrown_switch);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.thrown_switch * perf_vals.thrown_switch);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Cleared signals"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.cleared_signal, perf_vals.cleared_signal);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.cleared_signal * perf_vals.cleared_signal);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Reversed trains"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.turned_train, perf_vals.turned_train);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.turned_train * perf_vals.turned_train);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Missed station stops"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.nmissed_stops, perf_vals.nmissed_stops);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.nmissed_stops * perf_vals.nmissed_stops);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<tr><td valign=top>%s</td>n"), L("Wrong stock assignments"));
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td align=right valign=top>%d&nbsp;x</td><td align=right valign=top>%d&nbsp;=</td>"),
// 			    perf_tot.wrong_assign, perf_vals.wrong_assign);
// 	    page.Add(buff);
// 	    buff.Printf(wxT("<td valign=top>%d</td></tr>n"), perf_tot.wrong_assign * perf_vals.wrong_assign);
// 	    page.Add(buff);
...
// 	    if(outFormat == OUT_TEXT) {
// 		buff.Printf(wxT("%-10.10s  %-20.20s  %12s  %-20.20s  %12s %3s %3s %sn"),
...
// 	}
// 	page.EndTable();
// 	page.Add(wxT("<blockquote><blockquote>n"));
// 	if(t->days) {
...
// 		buff.Printf(wxT("%s.<br>n"), t->notes[status]);
// 		page.Add(buff);
// 	    }
// 	}
...
// 	    return 0;
// 	file.Write(wxString::Format(wxT("%sn"), current_project.c_str()));
// 
// 	file.Write(wxString::Format(wxT("%d,%ld,%d,%d,%d,%d,%d,%d,%d,%ldn"),
...
// 		continue;
// 	    file.Write(wxString::Format(wxT("%d,%d,%dn"), t->x, t->y, t->switched));
// 	}
// 	file.Write(wxString::Format(wxT("n")));
// 
...
// 		continue;
// 	    file.Write(wxString::Format(wxT("%sn"), tr->name));
// 	    file.Write(wxString::Format(wxT("  %d,%d,%sn"), tr->status, tr->direction,
...
// 	    }
// 	    file.Write(wxT("n"));
// 	    file.Write(wxString::Format(wxT("  %ld,%d,%g,%g,%dn"), tr->timedep, 0, //tr->pathpos,
...
// 	    file.Write(wxString::Format(wxT(",%dn"), tr->needfindstop));
// 	    if(tr->fleet && tr->fleet->_size) {
// 		/* file.Write(wxString::Format("  %d,%dn", tr->fleet->x, tr->fleet->y)); */
// 		file.Write(wxT("  "));
...
// 		file.Write(wxT('n'));
// 	    } else
// 		file.Write(wxString::Format(wxT("  0,0n")));	    /* length has fleet info at end */
// 	    if(tr->position)
// 		file.Write(wxString::Format(wxT("  %d,%d"), tr->position->x, tr->position->y));
// 	    else
// 		file.Write(wxString::Format(wxT("  0,0")));
// 	    file.Write(wxString::Format(wxT(",%d,%d,%dn"), tr->waittime, tr->flags, tr->arrived));
// 	    file.Write(wxString::Format(wxT("  %d,%sn"), tr->oldstatus,
// 				tr->outof ? tr->outof->station : wxT("")));
// 
//             if(tr->startDelay) {
//                 file.Write(wxString::Format(wxT(":startDelay %dn"), tr->startDelay));
//             }
// 
...
// 		if(!ts->depDelay)
// 		    file.Write(wxString::Format(wxT("    %s,%d,%dn"),
// 			    ts->station, ts->stopped, ts->delay));
// 		else
// 		    file.Write(wxString::Format(wxT("    %s,%d,%d,%dn"),
// 			    ts->station, ts->stopped, ts->delay, ts->depDelay->nSeconds));
// 	    }
// 	    if(tr->tail && tr->tail->path) {
// 		Train *tail = tr->tail;
// 
// 		file.Write(wxString::Format(wxT(".n")));	/* marks beginning of tail path */
// 		file.Write(wxString::Format(wxT("  %sn"), tr->stopping ? tr->stopping->station : wxT("")));
// 		if(tr->length) {
...
// 		     */
// 		    file.Write(wxString::Format(wxT("=length %dn"), tr->length));
// 		    file.Write(wxString::Format(wxT("=icons %d %dn"), tr->ecarpix, tr->wcarpix));
// 		}
...
// 	    file.Write(wxT("(white tracksn"));
// 	    for(t = layout; t; t = t->next) {
// 		if((t->type == TRACK || t->type == SWITCH) && t->fgcolor == color_white)
// 		    file.Write(wxString::Format(wxT("%d,%dn"), t->x, t->y));
// 	    }
// 	    file.Write (wxT(")n"));
// 	}
// 
...
// 
// 		    file.Write(wxString::Format(wxT(",%dn"), tr->tail->path->_size));
...
// 	file.Write (wxT(")n"));
// 
// 	file.Write(wxString::Format(wxT("%d,%d,%d,%d,%dn"), run_day, terse_status, status_on_top,
// 		show_seconds, signal_traditional));
// 	file.Write(wxString::Format(wxT("%d,%dn"), auto_link, show_grid));
// 	file.Write(wxString::Format(wxT("%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%dn"),
...
// 	file.Write(wxString::Format(wxT("%dn"), hard_counters));
// 	file.Write(wxString::Format(wxT("%dn"), show_canceled));
// 	file.Write(wxString::Format(wxT("%dn"), show_links));
// 	file.Write(wxString::Format(wxT("%dn"), beep_on_enter));
// 	file.Write(wxString::Format(wxT("%dn"), bShowCoord));
// 	file.Write(wxString::Format(wxT("%dn"), show_icons));
// 	file.Write(wxString::Format(wxT("%dn"), show_tooltip));
// 	file.Write(wxString::Format(wxT("%dn"), show_scripts));
// 	file.Write(wxString::Format(wxT("%dn"), random_delays));
// 	file.Write(wxString::Format(wxT("%dn"), link_to_left));
// 	file.Write(wxString::Format(wxT("%dn"), play_synchronously));
// 
// 	file.Close();
// 	return 1;
// }
...
// 	page.StartPage(buff);
// 	page.AddRuler();
// 	page.Add(wxT("<blockquote>n"));
// 	for(tl = track_info; tl; tl = tl->next) {
// 	    buff.Printf(wxT("%sn"), tl->txt);
// 	    page.Add(buff);
// 	}
// 	page.EndPage();
// }
